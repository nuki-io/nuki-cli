...
func TestAuthorizeContextSharedKey(t *testing.T) {
	...
	ac.SetCliKeys(cliPrivKey, nil)
	ac.SetSmartlockPublicKey(slPubKey)
	ac.CalculateSharedKey()

	require.Equal(t, []byte{0x91, 0x55, 0x61, 0x58, 0x7D, 0x86, 0x81, 0x5B, 0x70, 0x9E, 0xDD, 0x58, 0x19, 0xD8, 0xC6, 0xF2, 0xE8, 0x83, 0xDA, 0x3C, 0x86, 0xF4, 0x61, 0xF1, 0x3B, 0x84, 0x22, 0x8B, 0x84, 0x53, 0x3E, 0x04}, ac.GetSharedKey())
}

func TestAuthorizeContextAuthenticator(t *testing.T) {
	ac := &bleflows.AuthorizeContext{}
	ac.SetCliKeys(cliPrivKey, cliPubKey)
	ac.SetSmartlockPublicKey(slPubKey)
	ac.CalculateSharedKey()
	...
	authenticator := ac.GetMessageAuthenticator(ac.GetCliPublicKey(), ac.GetSmartlockPublicKey(), challenge)
